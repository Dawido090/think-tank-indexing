...
@op
def preprocess_raw_data(files_which_can_be_moved):
    client = get_minio_client()
    if check_minio_connection(client) == False:
        get_dagster_logger().error('Minio is down!')
    ...
    return raw_data_to_be_moved

# change to accept dict of data instead of list of names
@op
def move_raw_data_to_transformed(raw_data_to_be_moved):
    client = get_minio_client()
    if check_minio_connection(client) == False:
        get_dagster_logger().error('Minio is down!')
...
def transform_raw_data_job():
    ...
    pre_process = preprocess_raw_data(get_raw_data)
    move_raw_data = move_raw_data_to_transformed(pre_process)
